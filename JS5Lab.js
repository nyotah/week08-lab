...
let myGreeting = new Greeting('Nyota', 'Montana');
myGreeting.hello();
...
let yourBook = new Book('War and Peace', 'Leo Tolstoy');
...
let newStudent = new Student('Jacky', 'student');
let newTeacher = new Teacher('Mr. Bean', 'teacher');
...
let newChild = new Child('Pugsley', '10');
newChild.details();
...
class List {
  ...
  displayMovies() {
    ...
    for (let i = 0; i < this.movies.length; i++) {
      movieInformation.push(this.movies[i].title + ' ' + this.movies[i].director)
    }
    return movieInformation;
  }
}

let movie1 = new Movie('Jurrasic Park', 'Steven Spielburg');
